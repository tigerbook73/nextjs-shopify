...
| API 入口   | Storefront API（`/api/2024-10/graphql.json`） | Customer Account API（`/account/customer/api/2024-01/graphql`） |
...
API：   https://shopify.com/{SHOPIFY_SHOP_ID}/account/customer/api/2024-01/graphql
```
...
- `setTokenCookies(cookieStore, tokens)` — 写入 `ca_access_token` / `ca_refresh_token` / `ca_token_expiry`
- `getAccessToken(cookieStore)` — 读取 access token
- `isTokenExpired(cookieStore)` — 判断 `ca_token_expiry` 是否过期
- `clearTokenCookies(cookieStore)` — 清除所有 `ca_*` cookie
- `refreshAccessToken()` — 调用 token 端点换新 access token（`grant_type=refresh_token`）

**`src/lib/shopify/customer-account/client.ts`**

- `customerAccountFetch<T>(accessToken, query, variables?)` — 向 Customer Account API 发 GraphQL 请求，携带 Bearer token
...
- 逻辑：
  1. 无 access_token → `next/navigation` redirect 至 `/api/auth/login?return_to={pathname}`
  2. access_token 未过期 → `NextResponse.next()`（透传请求）
  3. access_token 已过期且有 refresh_token → 调用 `refreshAccessToken()` → 若成功，设置新 Cookie 后 `NextResponse.next()`；若失败，清除 Cookie 并 redirect 至 `/api/auth/login`
...
- 调用 `customerAccountFetch` + `GET_CUSTOMER_QUERY` 获取用户资料
- 调用 `GET_ORDERS_QUERY` 获取订单数（仅展示笔数）
- 保留 initials 头像逻辑
...
- `(auto)` `pnpm vitest run src/test/middleware.test.ts`
